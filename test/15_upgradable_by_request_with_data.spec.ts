...
describe('UpgradableByRequestWithData', () => {
  let address: Address;
  let upgradableExample: Contract<
    FactorySource['UpgradableByRequestWithDataExample']
  >;
  let upgraderExample: Contract<FactorySource['UpgraderWithDataExample']>;
  ...
  before('deploy contracts', async () => {
    ...
    const { contract } = await locklift.factory.deployContract({
      contract: 'UpgraderWithDataExample',
      publicKey: signer0.publicKey,
  ...
  describe('deploy upgradable by request instance', () => {
    it('should set instance code', async () => {
      const UpgradableByRequestExample = locklift.factory.getContractArtifacts(
        'UpgradableByRequestWithDataExample',
      );
  ...
  describe('request instance upgrade and check version and event', () => {
    ...
    it('should return extra value 123', async () => {
      const extra = await upgradableExample.methods
        .getExtra({ answerId: 0 })
        .call();
